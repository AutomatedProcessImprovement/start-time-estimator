...
def test_simple_resource_availability_bot_resources():
    ...
    # The availability of a bot resource is the same timestamp as checked
    artificial_event = {config.log_ids.end_time: pd.Timestamp('2006-11-07T17:00:00.000+02:00')}
    assert resource_availability.available_since('Dominic', artificial_event) == pd.Timestamp('2006-11-07T17:00:00.000+02:00')
...
def test_simple_resource_availability_considering_start_times():
    ...
    event_log = read_csv_log('./tests/assets/test_event_log_4.csv', config.log_ids, config.missing_resource)
    resource_availability = SimpleResourceAvailability(event_log, config)
...
def test_calendar_resource_availability():
    ...
    event_log = read_csv_log('./tests/assets/test_event_log_5.csv', config.log_ids, config.missing_resource)
    ...
    first_trace = event_log[event_log[config.log_ids.case] == 'trace-01']
    second_trace = event_log[event_log[config.log_ids.case] == 'trace-02']
    assert resource_availability.available_since('Marcus', first_trace.iloc[2]) == first_trace.iloc[0][config.log_ids.end_time]
    assert resource_availability.available_since('Marcus', second_trace.iloc[3]) == second_trace.iloc[1][config.log_ids.end_time]
    # The availability of the resource is the timestamp of its previous executed event
    artificial_event = {config.log_ids.end_time: pd.Timestamp("2022-01-03 11:30:00+00:00")}
    assert resource_availability.available_since('Marcus', artificial_event) == first_trace.iloc[0][config.log_ids.end_time]
    ...
    assert resource_availability.available_since('Marcus', artificial_event) == pd.Timestamp("2022-01-03 08:00:00+00:00")
    assert resource_availability.available_since('Marcus', first_trace.iloc[0]) == pd.Timestamp("2022-01-03 08:00:00+00:00")
    # The missing resource is always available (pd.NaT)
