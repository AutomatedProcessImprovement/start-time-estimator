...
class StartTimeEstimator:
    ...
    def _estimate_data_frame(self):
        ...
        if self.config.log_ids.start_time not in self.event_log.columns:
            self.event_log[self.config.log_ids.start_time] = pd.NaT
        ...
        self.event_log[self.config.log_ids.enabled_time] = np.where(
            ...
            self.event_log[self.config.log_ids.end_time],
            self.event_log[self.config.log_ids.start_time]
        )
        ...
        self.event_log[self.config.log_ids.start_time] = np.where(
            self.event_log[self.config.log_ids.activity].isin(self.config.instant_activities),
            self.event_log[self.config.log_ids.end_time],
            self.event_log[self.config.log_ids.start_time]
        )
        ...
        for (key, trace) in self.event_log.groupby([self.config.log_ids.case]):
            for index, event in trace.iterrows():
                if pd.isnull(event[self.config.log_ids.start_time]):
                    ...
                    available_time = self.resource_availability.available_since(
                        ...
                        event[self.config.log_ids.end_time]
                    )
                    ...
                    self.event_log.loc[index, self.config.log_ids.start_time] = max(enabled_time, available_time)
        ...
        for trace in self.event_log:
            for event in trace:
                if event[self.config.log_ids.activity] in self.config.instant_activities:
                    ...
                    event[self.config.log_ids.enabled_time] = event[self.config.log_ids.end_time]
                    event[self.config.log_ids.available_time] = event[self.config.log_ids.end_time]
                    event[self.config.log_ids.start_time] = event[self.config.log_ids.end_time]
                elif self.config.log_ids.start_time not in event:
                    ...
                    event[self.config.log_ids.start_time] = max(enabled_time, available_time)
        # Re-estimate start time of those events with an estimated duration over the threshold
        if not math.isnan(self.config.outlier_threshold):
            self._re_estimate_durations_over_threshold()
        # Fix start time of those events for which it could not be estimated (with [config.non_estimated_time])
    ...
    def _re_estimate_durations_over_threshold(self):
        if self.event_log_type == EventLogType.DATA_FRAME:
            ...
            statistic_durations = \
                self.event_log[self.event_log[self.config.log_ids.start_time] != self.config.non_estimated_time] \
                    .groupby([self.config.log_ids.activity]) \
                    .apply(lambda row: row[self.config.log_ids.end_time] - row[self.config.log_ids.start_time]) \
                    .groupby(level=0) \
            ...
            for index, event in self.event_log.iterrows():
                ...
                if (event[self.config.log_ids.start_time] != self.config.non_estimated_time and
                        (event[self.config.log_ids.end_time] - event[self.config.log_ids.start_time]) > duration_limit):
                    self.event_log.loc[index, self.config.log_ids.start_time] = \
                        event[self.config.log_ids.end_time] - duration_limit
        elif self.event_log_type == EventLogType.EVENT_LOG:
            # Take all the estimated durations of each activity
            statistic_durations = {}
            for trace in self.event_log:
                for event in trace:
                    if event[self.config.log_ids.start_time] != self.config.non_estimated_time:
                        # Store estimated time to calculate statistics
                        activity = event[self.config.log_ids.activity]
                        duration = event[self.config.log_ids.end_time] - event[self.config.log_ids.start_time]
                ...
                for event in trace:
                    if event[self.config.log_ids.start_time] != self.config.non_estimated_time:
                        ...
                        if duration > duration_limit:
                            # Set statistic
                            event[self.config.log_ids.start_time] = event[self.config.log_ids.end_time] - duration_limit
    ...
    def _set_instant_non_estimated_start_times(self):
        ...
        if self.event_log_type == EventLogType.DATA_FRAME:
            self.event_log[self.config.log_ids.start_time] = np.where(
                self.event_log[self.config.log_ids.start_time] == self.config.non_estimated_time,
                self.event_log[self.config.log_ids.end_time],
                self.event_log[self.config.log_ids.start_time]
            )
        elif self.event_log_type == EventLogType.EVENT_LOG:
        ...
        if self.event_log_type == EventLogType.DATA_FRAME:
            ...
            activity_durations = \
                self.event_log[self.event_log[self.config.log_ids.start_time] != self.config.non_estimated_time] \
                    ...
                    .apply(lambda row: row[self.config.log_ids.end_time] - row[self.config.log_ids.start_time])
            # Identify events with non_estimated as start time
            non_estimated_events = self.event_log[self.event_log[self.config.log_ids.start_time] == self.config.non_estimated_time]
            for index, non_estimated_event in non_estimated_events.iterrows():
                ...
                self.event_log.loc[index, self.config.log_ids.start_time] = \
                    non_estimated_event[self.config.log_ids.end_time] - duration
        elif self.event_log_type == EventLogType.EVENT_LOG:
            # Store the durations of the estimated ones
            ...
            for event in non_estimated_events:
                ...
                # Re-estimate
                duration = self._get_activity_duration(activity_durations, activity)
                event[self.config.log_ids.start_time] = event[self.config.log_ids.end_time] - duration
